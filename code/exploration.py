...
creation_date_errors = {'02/18/1605:12:00 AM PST': '02/18/16 05:12:00 AM PST',
                        '03/22/1607:17:38 AM PDT': '03/22/16 07:17:38 AM PDT',
                        '04/13/1607:31:13 AM PDT': '04/13/16 07:31:13 AM PDT',
                        '10/20/16 07:51:40AM PDT': '10/20/16 07:51:40 AM PDT',
                        '11/03/1608:21:10 AM PST': '11/03/16 08:21:10 AM PST',
                        '11/10/1607:17:06 AM PST': '11/10/16 07:17:06 AM PST',
                        '11/01/16 03:03:08 AM P': '11/01/16 03:03:08 AM PST',
                        '11/01/16 08:03:20 AM P': '11/01/16 08:03:20 AM PST',
                        '11/01/16 08:04:40 AM PST': '11/01/16 08:04:40 AM PST',
                        '12/07/1611:37:56 PM PST': '12/07/16 11:37:56 PM PST',
                        '12/08/1606:22:13 AM PST': '12/08/16 06:22:13 AM PST',
                        '12/27/1611:32:51 PM PST': '12/27/16 11:32:51 PM PST',
                        '05/16/1706:17:51 AM PDT': '05/16/17 06:17:51 AM PDT',
                        '05/17/17 04:21:04AM PDT': '05/17/17 04:21:04 AM PDT',
                        '02/07/1707:18:42 AM PST': '02/07/17 07:18:42 AM PST',
                        '02/06/1707:17:41 AM PST': '02/06/17 07:17:41 AM PST',
                        '03/14/17 07:01:00AM PDT': '03/14/17 07:01:00 AM PDT'}


# This dictionary will fix the errors found in the "enddate" column
...
def term_frequency_adjusted(col):
    if type(col) == str:
        token = nltk.word_tokenize(col)
        text = []
        document_length = len(col)
        for item in token:
            if item.isalnum():
                text.append(item)
            else:
        ...
        for i in frequencies:
            adjusted_frequencies[i] = frequencies[i]/document_length
...
# Here we will turn the text of each ad into a sparse vector
def text_to_sparse_vector():
    # First, we must concatenate list of all ad texts
    texts = [text for text in data_df['adtext']]

    # Next, we'll tokenize the corpus of texts and grab frequency of each word
    texts_tokens = []
    for i in texts:
        if type(i) == str:
            result = nltk.word_tokenize(i.lower())
            for r in result:
                if r.isalnum():
                    texts_tokens.append(r)
        else:
            continue
    texts_tokens_freqs = dict(Counter(texts_tokens))
    sorted_text_token_freqs = sorted(texts_tokens_freqs.items(), key=lambda kv: kv[1], reverse=True)
    print(type(sorted_text_token_freqs))
    print(sorted_text_token_freqs[0:5])
    sorted_tokens_list = [item[0] for item in sorted_text_token_freqs]
    print(sorted_tokens_list[0:5])

    # Finally, we construct a sparse vector representation of each ad's text
    all_ad_vectors = []
    for ad in data_df['adtext']:
        if type(ad)==str:
            tokenized_ad = nltk.word_tokenize(ad.lower())
            ad_vector = np.zeros(len(sorted_tokens_list))
            for word in sorted_text_token_freqs:
                if word in tokenized_ad:
                    word_index = sorted_tokens_list.index(word)
                    ad_vector[word_index] = 1
            all_ad_vectors.append(ad_vector)
    data_df['ad_vector'] = all_ad_vectors
    return data_df


# dla_tf -> document-length-adjusted term frequency
data_df['dla_tf'] = data_df['adtext'].map(term_frequency_adjusted)
text_to_sparse_vector()
print(data_df[['dla_tf', 'ad_vector']].head())
